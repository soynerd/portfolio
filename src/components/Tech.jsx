...
export default function TechGrid() {
  ...
  const runAnimationSequence = async () => {
    ...
    for (const tech of sortedTechs) {
      ...
      setTechItems((prevItems) =>
        prevItems.map((item) =>
          item.name === tech.name
            ? { ...item, isAnimating: false, selected: false, position: 0 }
            : item
        )
      );
      await delay(200);
  ...
  const handleClick = (name) => {
    ...
    if (!clickedItem.selected) {
      setTechItems((prevItems) =>
        prevItems.map((item) =>
          item.name === name
            ? { ...item, selected: true, position: newOrder }
            : item
        )
      );
  ...
  return (
    <div className="max-w-4xl mx-auto ">
      <div className="flex flex-col items-center justify-center bg-amber-50 dark:bg-gray-900 p-4 transition-colors duration-300 rounded-2xl mb-10">
              ...
              <button
                ...
                className={clsx(
                  ...
                  tech.selected
                    ...
                    : "hover:border-gray-700 dark:hover:border-gray-400",
                  tech.isAnimating &&
                    "transform scale-125 z-10 shadow-2xl bg-indigo-500 text-black dark:text-white dark:bg-indigo-500",
